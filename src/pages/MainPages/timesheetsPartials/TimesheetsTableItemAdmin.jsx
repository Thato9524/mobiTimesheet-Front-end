...
function TimesheetsTableItemAdmin(props) {
  ...
  const typeIcon = (type) => {
    switch (type) {
      case "Subscription":
        return (
          <svg
            className="w-4 h-4 fill-current text-zinc-400 shrink-0 mr-2"
            viewBox="0 0 16 16"
          >
  ...
  return (
    <tbody className="text-sm">
      ...
      <tr>
        ...
        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
          <div className="text-left">{props.totalDayHours}</div>{" "}
          {/* WAS "text-left"  WITH NO "ml-5"*/}
        </td>
        ...
        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
          <div
            className={`inline-flex font-medium rounded-full text-left px-2.5 py-0.5 ${statusColor(
              props.status
            )}`}
          >
            {props.status}
        ...
        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
          <div className="flex items-center">
           {/*typeIcon(props.type) make it description*/}
            ...
            <button onClick={(e) => props.editTimesheet(e, props.id)}>
              {' '}
...
export default TimesheetsTableItemAdmin;
