...
function UsersTiles() {
  ...
  useEffect(async () => {
    ...
    let getCompletedsheet = await request.get(
      `${config.path.completedsheet.getCompletedsheet}/${_id}`
    );

    await setCompletedsheetData(getCompletedsheet);
    //GET MONTHS
    let getData = await request.get(config.path.timesheet.month);
    // // console.log(gotData);
    if (getData.err) {
      return;
    }
              ...
              data.map((item) => {
                ...
                return (
                  <UsersTilesCard
                    ...
                    id={item._id}
                    name={item.name}
                    link={`/timesheet/${item._id}`} //---------> Orders
                    data={item}
                    desc={item.desc}
                    linkTitle="Edit Timesheets"
